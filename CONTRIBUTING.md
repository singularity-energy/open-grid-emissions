...
- **Blank and zero values are not interchangeable**: a zero means the value is actually zero (reported as zero, or known to be zero), while a blank (`NaN`/`NA`) means the value is missing, unknown, or not applicable. For example, NOx emissions are zero for a solar generator, blank for a combustion generator that didn't report fuel consumption, and energy storage columns are blank for generators that aren't energy storage. Keep this distinction all the way through to the published outputs, and never convert blanks to zeros as a side effect of another operation (aggregation, merging, reshaping). When a blank should become a zero, fill it explicitly in a dedicated step, restricted to the rows where zero is actually known to be correct, with a comment explaining why (e.g. `data_cleaning.fill_emissions_for_non_emitting_resources`, or the `columns_to_fill_with_zero` argument to `data_cleaning.complete_hourly_timeseries`).
- `sum(..., min_count=1)` **is the default**: pandas' `sum()` returns 0 when every value in a group (or column) is missing, which silently turns blanks into zeros. Pass `min_count=1` when summing data, whether on a `groupby` (`df.groupby(keys, dropna=False)[cols].sum(min_count=1)`, combined with `numeric_only=True` where needed) or on a whole column/dataframe, so that a group with no data stays blank while a group with at least one value is summed as usual. `min_count=1` can be left off only when converting blanks to zeros is the intended behavior (e.g. logic that needs a value in every column), and that choice must be noted in an inline comment at the call site explaining why, so a plain `sum()` is never ambiguous about whether the conversion was intentional.
