...
def test_load_emissions_controls_eia923_pre_2012(load_data):
    ...
    load_emissions_controls_helper(
        list(reversed(range(2008, 2012))), load_data, expect_empty=True
    )


def test_load_boiler_nox_association_eia860(load_data):
    load_boiler_nox_association_eia860_helper(
        list(reversed(range(2013, 2021))), load_data
    )
    load_boiler_nox_association_eia860_helper(
        list(reversed(range(2005, 2013))), load_data, expect_empty=True
    )


def test_load_boiler_so2_association_eia860(load_data):
    load_boiler_so2_association_eia860_helper(
        list(reversed(range(2013, 2021))), load_data
    )
    load_boiler_so2_association_eia860_helper(
        list(reversed(range(2005, 2013))), load_data, expect_empty=True
