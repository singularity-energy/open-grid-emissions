...
def add_egrid_plant_id(df, from_id, to_id):
    ...
    id_map = dict(zip(list(egrid_crosswalk[f'plant_id_{from_id}']), list(egrid_crosswalk[f'plant_id_{to_id}'])))
...
def test_for_negative_values(df, columns_to_test):
    for column in columns_to_test:
        ...
        if not negative_test.empty:
            print(f'Warning: There are {len(negative_test)} records where {column} is negative. Check `negative_test` for complete list')
...
def test_chp_allocation(df):
    chp_allocation_test = df[df['fuel_consumed_for_electricity_mmbtu'] > df['fuel_consumed_mmbtu']]
    if not chp_allocation_test.empty:
        print(f'Warning: There are {len(chp_allocation_test)} records where fuel consumed for electricity is greater than total fuel consumption. Check `chp_allocation_test` for complete list')

    return chp_allocation_test

def test_for_missing_co2(df):
    missing_co2_test = df[df['co2_mass_tons'].isna() & ~df['fuel_consumed_mmbtu'].isna()]
    if not missing_co2_test.empty:
        print(f'Warning: There are {len(missing_co2_test)} records where co2 data is missing. Check `missing_co2_test` for complete list')
...
def test_for_missing_data(df, columns_to_test):
    ...
    if not missing_data_test.empty:
        print(f'Warning: There are {len(missing_data_test)} records for which no data was reported. Check `missing_data_test` for complete list')
...
def test_for_outlier_heat_rates(df):
    ...
    thermal_generators = df[~df['energy_source_code'].isin(["SUN", "MWH", "WND", "WAT", "WH", "PUR"])]
    for fuel_type in sorted(list(thermal_generators.energy_source_code.dropna().unique())):
        generators = thermal_generators[thermal_generators['energy_source_code'] == fuel_type]
        heat_rate = (generators['fuel_consumed_for_electricity_mmbtu'] / generators['net_generation_mwh'])
        heat_rate_stats = heat_rate[(heat_rate >= 0) & (heat_rate != np.inf)].describe()
        outlier_threshold = ((heat_rate_stats['75%'] - heat_rate_stats['25%']) * 1.5) + heat_rate_stats['75%']
        heat_rate_test = generators[(heat_rate > outlier_threshold) & (heat_rate != np.inf) & (generators['net_generation_mwh'] >= 1)]
