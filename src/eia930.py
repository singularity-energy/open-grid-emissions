...
def get_columns(ba: str, columns):
    ...
    DEM_ID = "EBA.{}-ALL.D.H"
    SRC = ["COL", "NG", "NUC", "OIL", "OTH", "SUN", "UNK", "WAT", "WND", "GEO",
           "BIO"]

    cols = [GEN_TYPE_ID.format(ba, f)
            for f in SRC if GEN_TYPE_ID.format(ba, f) in columns]
...
def get_int_columns(ba1: str, columns, ba2: list = []):
    ...
    if ba2 == []:
        other_cols = [c for c in columns
                      if re.split(r"[-.]", c)[1] == ba1
                      and re.split(r"[-.]", c)[2] != "ALL"]
        ba2 = [re.split(r"[-.]", c)[2] for c in other_cols]
        ba2.append("ALL")

    cols = [INTER_ID.format(ba1, ba) for ba in ba2 if
            (INTER_ID.format(ba1, ba) in columns)]
    if "ALL" in ba2:
...
def manual_930_adjust(raw: pd.DataFrame):
    ...
    sc_offsets = raw.index.tz_convert("US/Eastern").to_series().\
        apply(lambda s: s.utcoffset())
    ...
    cols = get_int_columns("PJM", raw.columns,
                           ["CPLE", "CPLW", "DUK", "LGEE", "MISO", "NYIS",
                            "TVA"])
    ...
    is_dst = raw.index.tz_convert("US/Eastern").to_series()\
        .apply(lambda s: s.utcoffset()) == timedelta(hours=-4)
    pjm_offset = [timedelta(hours=-3) if is_d else timedelta(hours=-4)
                  for is_d in is_dst]

    # make new data so we don't mess up other data indexing
    pjm_dat = raw[get_int_columns("PJM", raw.columns,
                                  ["CPLE", "CPLW", "DUK", "LGEE", "MISO",
                                   "NYIS", "TVA", "ALL"])].copy()
    # make shifted dates
