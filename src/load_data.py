    ...
    def download_pudl(zenodo_url, pudl_version):
        ...
        with open("../data/pudl.tgz", 'wb') as fd:
            for chunk in r.iter_content(chunk_size=block_size):
                print(
                    f'Downloading PUDL. Progress: {(round(downloaded/total_size_in_bytes*100,2))}%   \r', end='')
                fd.write(chunk)
    ...
    for year in years_to_download:
        for period in ['Jan_Jun', 'Jul_Dec']:
            if os.path.exists(f'../data/eia930/EIA930_BALANCE_{year}_{period}.csv'):
                ...
                r = requests.get(
                    f"https://www.eia.gov/electricity/gridmonitor/sixMonthFiles/EIA930_BALANCE_{year}_{period}.csv", stream=True)
...
def load_cems_data(year):
    ...
    cems['gross_generation_mwh'] = cems['gross_load_mw'] * \
        cems['operating_time_hours']
    ...
    cems['co2_mass_tons'] = cems['co2_mass_tons'] * \
        cems['operating_time_hours']
    cems['fuel_consumed_mmbtu'] = cems['fuel_consumed_mmbtu'] * \
        cems['operating_time_hours']

    # add a unique unit id
    cems['cems_id'] = cems['plant_id_eia'].astype(
        str) + "_" + cems['unitid'].astype(str)
...
def create_flat_eia930_placeholder_profiles(monthly_eia_data_to_distribute, energy_source_groups):
