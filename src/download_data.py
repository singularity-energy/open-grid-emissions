...
def download_raw_eia860(year):
    """
    Downloads the EIA-860 excel files
    Inputs:
        egrid_files_to_download: a list of urls for the egrid excel files that you want to download
    """
    # if there is not yet a directory for egrid, make it
...
def download_raw_eia861(year):
    """Downloads the EIA-861 excel files for use in grid gross loss calculations."""

    # if there is not yet a directory for egrid, make it
    if not os.path.exists("../data/downloads/eia861"):
        os.mkdir("../data/downloads/eia861")
    ...
    filename = url.split("/")[-1].split(".")[0]
    # if the file already exists, do not re-download it
    if os.path.exists(f"../data/downloads/eia861/{filename}"):
        print(f"    {year} EIA-861 already downloaded")
    else:
        print(f"    Downloading {year} EIA-861 data")
        try:
            r = requests.get(url, stream=True)

            with open(f"../data/downloads/eia861/{filename}.zip", "wb") as fd:
                for chunk in r.iter_content(chunk_size=1024):
                    fd.write(chunk)

            # Unzip
            with zipfile.ZipFile(
                f"../data/downloads/eia861/{filename}.zip", "r"
            ) as zip_to_unzip:
                zip_to_unzip.extractall(f"../data/downloads/eia861/{filename}")
            os.remove(f"../data/downloads/eia861/{filename}.zip")
        except:
            r = requests.get(archive_url, stream=True)
