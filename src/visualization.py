...
def day_hour_heatmap(timeseries:pd.Series, year:int=2021):
    ...
    hours_index = pd.DataFrame(index=pd.date_range(f"{year}-01-01 T00:00",f"{year}-12-31 T23:00", freq="H").tz_localize("EST"))
    hours_index = hours_index.merge(timeseries, how='left', left_index=True, right_index=True)
