...
def output_intermediate_data(df, file_name, path_prefix, year, skip_outputs):
    ...
    if not skip_outputs:
        ...
        df.to_csv(f"{outputs_folder()}{path_prefix}{file_name}_{year}.csv", index=False)
...
def output_to_results(df, file_name, subfolder, path_prefix, skip_outputs):
    if not skip_outputs:
        ...
        df.to_csv(
            f"{results_folder()}{path_prefix}{subfolder}us_units/{file_name}.csv",
            index=False,
        )
        metric.to_csv(
            f"{results_folder()}{path_prefix}{subfolder}metric_units/{file_name}.csv",
            index=False,
        )
...
def output_data_quality_metrics(df, file_name, path_prefix, skip_outputs):
    if not skip_outputs:
        ...
        df.to_csv(
            f"{results_folder()}{path_prefix}data_quality_metrics/{file_name}.csv",
            index=False,
        )
...
def write_plant_metadata(
    ...
    if not skip_outputs:
        ...
        metadata.to_csv(f"{results_folder()}{path_prefix}plant_data/plant_metadata.csv")
